...
#define CMD_JRIO_START   0x08   // JR.IO — start 2.4GHz disruptor on ESP1
#define CMD_JRIO_STOP    0x09   // JR.IO — stop disruptor, restore radios
...
enum St {
  ST_SPLASH,
  ST_MENU,
  ST_JRIO_STATUS, ST_JRIO_ACTIVE,         // JR.IO — first/main feature
...
const char* menuItems[]={
  "  JR.IO MODE",    // 0 ← main feature, listed first
  "  WIFI  SCAN",    // 1
  "  BLE   SCAN",    // 2
  "  NRF   SCAN",    // 3
  "  ALERT MODE"     // 4
};
int mCur=0, mScroll=0;

// ── JR.IO state ───────────────────────────────────────────────
bool jrioActive=false;
...
void drawMenu(){
  ...
  // Live dots: jrioActive = red-style blink, alertActive = dot
  if(jrioActive)  { oled.fillRect(114,2,12,7,SSD1306_BLACK); oled.setCursor(115,2); oled.print("TX"); }
  if(alertActive) { oled.fillCircle(122,5,3,SSD1306_BLACK); }

  oled.setTextColor(SSD1306_WHITE);
  for(int i=0;i<MENU_ROWS;i++){
    int idx=mScroll+i; if(idx>=MENU_COUNT) break;
    int y=12+i*13; bool sel=(idx==mCur);
    if(sel) oled.fillRect(0,y,128,12,SSD1306_WHITE);
    oled.setTextColor(sel?SSD1306_BLACK:SSD1306_WHITE);
    oled.setCursor(4,y+2);
    oled.print(sel?"> ":"  ");
    oled.print(menuItems[idx]);
    // Status indicators
    if(idx==0&&jrioActive)  { oled.setCursor(112,y+2); oled.print("*"); }
    if(idx==4&&alertActive) { oled.setCursor(112,y+2); oled.print("*"); }
    oled.setTextColor(SSD1306_WHITE);
  }
  // Scroll arrows
  if(mScroll>0)
    oled.fillTriangle(120,12,124,12,122,9,SSD1306_WHITE);
  if(mScroll+MENU_ROWS<MENU_COUNT)
    oled.fillTriangle(120,63,124,63,122,66,SSD1306_WHITE);
  oled.display();
}

// ═══════════════════════════════════════════════════════════════
//  SCREEN: JR.IO STATUS (inactive entry)
// ═══════════════════════════════════════════════════════════════
void drawJrioStatus(){
  oled.clearDisplay();
  hdr("JR.IO  MODE"); statusBadge(jrioActive);
  oled.setTextColor(SSD1306_WHITE); oled.setTextSize(1);
  oled.drawFastHLine(0,13,128,SSD1306_WHITE);

  if(jrioActive){
    oled.setCursor(4,18); oled.print("Status: RUNNING");
    oled.setCursor(4,28); oled.print("Both NRF24 @ MAX PWR");
    oled.setCursor(4,38); oled.print("ESP1 slide SW:");
    oled.setCursor(4,46); oled.print("HI=sweep  LO=random");
  } else {
    oled.setCursor(4,18); oled.print("2.4GHz disruptor");
    oled.setCursor(4,28); oled.print("Both NRF24 MAX power");
    oled.setCursor(4,38); oled.print("SW-HI:sweep");
    oled.setCursor(68,38); oled.print("SW-LO:random");
    oled.setCursor(4,48); oled.print("OK:activate");
  }
  bottomHint(jrioActive?"OK:stop  HOLD:menu":"OK:start  HOLD:menu");
  oled.display();
}

// ═══════════════════════════════════════════════════════════════
//  SCREEN: JR.IO ACTIVE (live disruptor view)
// ═══════════════════════════════════════════════════════════════
void drawJrioActive(){
  static uint32_t lt=0; static uint8_t fl=0;
  if(millis()-lt>300){ fl++; lt=millis(); }

  oled.clearDisplay();

  // Flashing outer border — aggressive pulse
  if(fl%2==0){
    oled.fillRect(0,0,128,12,SSD1306_WHITE);
    oled.drawRect(0,12,128,52,SSD1306_WHITE);
  } else {
    oled.drawRect(0,0,128,12,SSD1306_WHITE);
    oled.fillRect(0,0,128,12,SSD1306_BLACK);
    oled.drawRect(0,12,128,52,SSD1306_WHITE);
  }

  // Title
  oled.setTextColor((fl%2==0)?SSD1306_BLACK:SSD1306_WHITE);
  oled.setCursor(10,2); oled.print("!! JR.IO  ACTIVE !!");

  oled.setTextColor(SSD1306_WHITE); oled.setTextSize(1);

  // Animated channel counter (visual only)
  static uint8_t chDisp=0; chDisp=(chDisp+3)%80;
  oled.setCursor(4,15);  oled.print("HSPI ch:"); oled.print(chDisp);
  oled.setCursor(70,15); oled.print("VSPI ch:"); oled.print((chDisp+37)%80);

  oled.setCursor(4,25);  oled.print("PA level : MAX (+20dBm)");
  oled.setCursor(4,35);  oled.print("Data rate: 2 Mbps");
  oled.setCursor(4,45);  oled.print("Mode:SW-HI=sweep LO=rnd");
...
// ── JR.IO launchers ──────────────────────────────────────────
void launchJrio(){
  sendCmd(CMD_JRIO_START);
  jrioActive=true;
  st=ST_JRIO_ACTIVE;
}
void stopJrioCmd(){
  sendCmd(CMD_JRIO_STOP);
  jrioActive=false;
  st=ST_JRIO_STATUS;
  drawJrioStatus();
}
...
void drawAlertStatus(){
  ...
  if(alertActive){
    ...
    oled.setCursor(2,40);
    const char* mnames[]={"","WiFi SSID","BLE Beacon","WiFi+BLE All"};
...
void drawMethodSelect(){
  ...
  const char* methods[]={"WiFi SSID Only","BLE Beacon Only","Blast All(WiFi+BLE)"};
...
void drawAlertActive(){
  ...
  if(methodCur==2){ oled.setCursor(2,38); oled.print("WiFi+BLE both active"); }
  bottomHint("OK:stop broadcast");
  oled.display();
}
...
void launchAlert(){
  ...
  if(methodCur==2) cmd=CMD_ALERT_ALL;
...
void loop(){
  ...
  case ST_MENU:
    if(up&&mCur>0){
      mCur--;
      if(mCur<mScroll) mScroll=mCur;
      drawMenu();
    }
    if(dn&&mCur<MENU_COUNT-1){
      mCur++;
      if(mCur>=mScroll+MENU_ROWS) mScroll=mCur-MENU_ROWS+1;
      drawMenu();
    }
    if(mid){
      switch(mCur){
        case 0: st=ST_JRIO_STATUS;  drawJrioStatus();               break;
        case 1: st=ST_WIFI_STATUS;  drawStatusScreen("WIFI SCAN",false); break;
        case 2: st=ST_BLE_STATUS;   drawStatusScreen("BLE  SCAN",false); break;
        case 3: st=ST_NRF_STATUS;   drawStatusScreen("NRF  SCAN",false); break;
        case 4: st=ST_ALERT_STATUS; drawAlertStatus();              break;
      }
    }
    break;
  ...
  // ── JR.IO ──────────────────────────────────────────────────
  case ST_JRIO_STATUS:
    if(!jrioActive&&mid){ launchJrio(); }
    if(jrioActive&&mid){  stopJrioCmd(); }
  ...
  case ST_JRIO_ACTIVE:
    drawJrioActive();
    if(mid){ stopJrioCmd(); }
    if(midH){ stopJrioCmd(); st=ST_MENU; drawMenu(); }
    break;
